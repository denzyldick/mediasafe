...
pub fn start_background_worker(app: &AppHandle, config_path: String) -> (UnboundedSender<String>, Arc<AtomicUsize>) {
    ...
    std::thread::spawn(move || {
        ...
        let mut tokenizer: Option<tokenizers::Tokenizer> = None;
        let mut precomputed_text_embeddings: Vec<(String, Vec<f32>)> = Vec::new();
        ...
        while let Some(photo_id) = rx.blocking_recv() {
            ...
            if photo_id.starts_with("__VIDEO_FRAMES__:") {
                let parts: Vec<&str> = photo_id.split("|||").collect();
                if parts.len() > 1 {
            ...
            let _ = app_handle.emit("indexing-progress", current.saturating_sub(1));
        }
    });
    (tx, pending_count)
}
