...
#[tauri::command]
async fn list_files(app: tauri::AppHandle, offset: usize, limit: usize, query: String, scan: bool, favorites_only: bool, videos_only: bool) -> Result<String, String> {
    let path = get_config_path(&app);
    if path.is_empty() { return Ok("[]".to_string()); }
    if scan { scan_files(app.clone()); }
    let database = database::Database::new(&path);
    Ok(serde_json::to_string(&database.list_photos(&query, offset, limit, favorites_only, videos_only)).unwrap_or("[]".to_string()))
}
...
async fn get_person_photos(app: tauri::AppHandle, person_id: String) -> Result<String, String> {
    ...
    if path.is_empty() { return Ok("[]".to_string()); }
    let database = database::Database::new(&path);
    Ok(serde_json::to_string(&database.get_photos_for_person(&person_id)).unwrap_or("[]".to_string()))
}
...
async fn get_top_tags(app: tauri::AppHandle) -> String {
    ...
    if let Ok(mut stmt) = database.connection.prepare("SELECT class FROM object GROUP BY class ORDER BY COUNT(*) DESC LIMIT 5") {
        if let Ok(iter) = stmt.query_map([], |row| row.get(0)) {
            for item in iter.flatten() { tags.push(item); }
        }
    }
...
async fn index_faces(app: tauri::AppHandle, state: tauri::State<'_, ml::MlContext>) -> Result<(), String> {
    ...
    if let Ok(tx) = state.tx.lock() {
        ...
        let total = state.pending_count.fetch_add(count, std::sync::atomic::Ordering::SeqCst) + count;
        let _ = app.emit("indexing-progress", total);
        for id in photo_ids { let _ = tx.send(id); }
    }
    Ok(())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let (tx, pending_count) = ml::start_background_worker(app.handle(), config_path.clone());
